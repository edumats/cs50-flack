...
messagesArchive = {
    "General": collections.deque([], maxlen=100)
}
...
def channel(data):
    ...
    messagesArchive.setdefault(channel, collections.deque([], maxlen=100))
...
def joinChannel(data):
    ...
    if selectedChannel == 'empty':
        ...
        try:
            ...
            emit(
                'alert message',
                {'message': "Saved channel does not exist, please log off"},
                room=request.sid
            )
        ...
        emit(
            'return message',
            {
                'messageField': 'has joined the room ' + currentChannel,
        ...
        emit(
            'return message',
            {
                'messageField': 'has left the room ' + currentChannel,
        ...
        emit(
            'return message',
            {
                'messageField': 'has joined the room ' + selectedChannel,
                'currentChannel': selectedChannel,
                'currentTime': data.get('currentTime'),
                'user': data.get('user')
            },
...
# Must add line below because of SocketIO bug:
